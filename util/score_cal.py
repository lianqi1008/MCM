...
parser.add_argument('-d2','--dir_score', default='./dataset/coco/scores', help='Path to output score', type=str)
...
if __name__ == '__main__':
    ...
    with open(os.path.join(opt.dir_score, 'texture.csv'), 'a', encoding='utf-8', newline='') as f:
        with open(os.path.join(opt.dir_score, 'structure.csv'), 'a', encoding='utf-8', newline='') as f1:
            ...
            texture_list = list(range(256))
            texture_list.insert(0, 'image')

            structure_list = list(range(256))
            structure_list.insert(0, 'image')

            cw.writerows([texture_list])
            ...
            for i in range(len(img_names)):
                ...
                texture_scores = cal_texture(image, 16, 16)
                structure_scores = cal_structure(label, 16, 16)

                texture_scores.insert(0, img_name)
                cw.writerows([texture_scores])
